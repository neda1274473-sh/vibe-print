"""
Pre-hardware verification script — runs 6 different natural-language requests
through the full pipeline and captures real intermediate output at every stage.
"""
...
# Ensure vibe_print is importable
...
# Test requests
REQUESTS = [
    "I need a squeezer for a 40mm diameter tube of toothpaste",
    "Can you make a mounting bracket with two screw holes, about 60mm tall?",
    "I want a small open box, 50x30x20mm, to organize screws on my desk",
    "A hook that can hold a coat, mounted on a wall",
    "A spacer ring, 10mm outer diameter, 4mm hole in the middle",
    "Something to hold my stuff on the desk",  # deliberately vague
]


def run_request(goal: str, idx: int):
    """Run a single request through the full pipeline and return all intermediate data."""
    print(f"\n{'='*70}")
    ...
    # --- Step 3: Model path and analysis ---
...
def main():
    ...
    # Save raw results
    out_path = Path("verification_raw_results.json")
